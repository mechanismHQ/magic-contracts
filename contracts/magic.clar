...
(define-read-only (read-uint32-v2 (num (buff 4)))
  (let
    (
      (length (len num))
      (byte-1 (buff-to-u8 (unwrap! (element-at num u0) ERR_READ_UINT)))
      (byte-2 (if (> length u1) (buff-to-uint-le (unwrap-panic (element-at num u1))) u0))
      (byte-3 (if (> length u2) (buff-to-uint-le (unwrap-panic (element-at num u2))) u0))
      (byte-4 (if (> length u3) (buff-to-uint-le (unwrap-panic (element-at num u3))) u0))
    )
    (ok (+ (* byte-4 u16777216) (* byte-3 u65536) (* byte-2 u256) byte-1))
  )
)
